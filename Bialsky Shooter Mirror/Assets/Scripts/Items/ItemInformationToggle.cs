...
{
    public class ItemInformationToggle : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        [SerializeField] GameObject itemInformationDisplayPrefab = default;
        ItemDisplay itemDisplay;
        GameObject itemInformationDisplayInstance;
        IItemSelection itemSelection;

        private void Update()
        {
            if (itemSelection == null) itemSelection = GetComponent<IItemSelection>();
            if(itemSelection.GetItemId() == Guid.Empty && itemDisplay != null)
            {
                SetItemDisplay(null);
